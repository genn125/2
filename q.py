...
#
#         # Заголовок с отображением выбранных форматов (ГЛАВНАЯ ХРЕНЬ, ОТВЕЧАЕТ ЗА РАСПОЛОЖЕНИЕ ВСЕГО ТЕКСТА СВЕРХУ)
#         header_frame = tk.Frame(self.root, bg="#f0f0f0", padx=10, pady=10)
#         header_frame.pack(fill=tk.X)
#
#
#         # БЫЛО: Создаем отдельный Label для текста с форматами с подчеркиванием и жирным
# # СТАЛО: Метка "ПОИСК:" жирным шрифтом
...
#         formats_frame = tk.Frame(header_frame, bg="#f0f0f0")
#         formats_frame.pack(side=tk.LEFT, padx=(5, 0))
#
#
#         # Метка для отображения форматов (будет обновляться)
#         self.formats_text = tk.Text(
#             formats_frame,
#             height=4,  # Достаточно для нескольких строк
...
#
#             if formats:
#                 lines.append(f"{group}: {', '.join(formats)}")
#
#         # Объединяем строки с переносами
#         return '\n'.join(lines)
#
#         # # Определяем максимальную длину для одной строки (было до того как сделал перенос каждой категории с новой
#         # строки)
#         # max_line_length = 50
